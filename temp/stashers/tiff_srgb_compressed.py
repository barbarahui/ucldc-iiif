...
class TiffNuxeoStashRef(NuxeoStashRef):

    def __init__(self, path, bucket, pynuxrc):
        super(TiffNuxeoStashRef, self).__init__(path, bucket, pynuxrc)

    def _create_jp2(self):

        ''' this works for UCM/Ramicova, UCI/Gearhardt, UCI/McMillan '''
...
def main(argv=None):
    ...
    nxstash = TiffNuxeoStashRef(argv.path, argv.bucket, argv.pynuxrc)
